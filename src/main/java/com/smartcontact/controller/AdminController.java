...
public class AdminController {
    ...
    public String adminStausUpdate(
        ...
        log.info("ADMIN is: {} ",loggedEmail);
        User user = userService.findByUserId(id);

        // Admin cannot modify himself
        if (loggedEmail.equals(user.getEmail())) {
            ra.addFlashAttribute("msg", "Action Restricted: You cannot modify your own account!");
            ra.addFlashAttribute("type", "error");
        ...
        if ("ROLE_ADMIN".equals(user.getRole())) {
            ra.addFlashAttribute("msg", "Action Restricted: You cannot modify another admin!");
            ra.addFlashAttribute("type", "error");
            return "redirect:/admin/manage_users";
        }
